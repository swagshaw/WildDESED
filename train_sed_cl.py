...
from sed_trainer_cl import SEDTask4
from resample_folder import resample_folder
from utils import generate_tsv_wav_durations
    ...
    for step in range(1,6):
        ...
        if test_state_dict is None:
            ...
            if step != 1:
                ...
                synth_set = StronglyAnnotatedSet(config["data"]["synth_folder"],
                                                subsets[0],
                                                encoder,
                                                pad_to=config["data"]["audio_max_len"])
            ...
            if step != 1:
                ...
                synth_val = StronglyAnnotatedSet(config["data"]["synth_val_folder"],
                                                    subsets[0],
                                                    encoder,
                                                    return_filename=True,
                                                    pad_to=config["data"]["audio_max_len"])
            ...
            for i, noise_level in enumerate(noise_levels):
                if i>0:
                    synth_val_set_list.append(StronglyAnnotatedSet(os.path.join(config["data"]["noise_val_folder"], f"val{noise_level}"),
                                                                        subsets[i],
                                                                        encoder,
                                                                        return_filename=True,
                                                                        pad_to=config["data"]["audio_max_len"]))
            ...
            tot_val_data = [synth_val, weak_val]
            valid_dataset = torch.utils.data.ConcatDataset(tot_val_data)
