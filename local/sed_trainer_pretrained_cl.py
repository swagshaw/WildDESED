...
class SEDTask4(pl.LightningModule):
    ...
    def __init__(
        ...
        self.max_epochs = self.hparams["training"]["n_epochs"]
        self.opt = opt
    ...
    def validation_step(self, batch, batch_indx):
        ...
        mask_synth = (
            torch.tensor(
                [
                    (str(Path(x).parent)
                    == str(Path(self.hparams["data"]["synth_val_folder"]))) or (str(Path(x).parent.parent) == str(Path(self.hparams["data"]["noise_val_folder"])))
                    for x in filenames
                ]
            )
            .to(audio)
        ...
        if torch.any(mask_synth):
            ...
            filenames_synth = [
                x
                for x in filenames
                if Path(x).parent == Path(self.hparams["data"]["synth_val_folder"]) or Path(x).parent.parent == Path(self.hparams["data"]["noise_val_folder"])
            ]
            try:
                assert len(filenames_synth) == len(strong_preds_student[mask_synth])
            except AssertionError:
                print(
                    f"filenames_synth: {len(filenames_synth)} and strong_preds_student[mask_synth]: {len(strong_preds_student[mask_synth])} are not equal"
                )
    ...
    def on_validation_epoch_end(self, outputs=None):
        ...
        self.log(f"val/task_{self.step}/obj_metric", obj_metric, prog_bar=True)
    ...
    def test_step(self, batch, batch_indx):
        ...
        #     with open('/home/yinhan/codes/sep4noiseSED/weak_predictions/test_-5db_sednoise_nocl.txt', 'a+') as f:
        #         f.write(wr_filename)
        #         f.write('\t')
        #         f.write(sound_event)
        #         f.write('\n')
    ...
    def on_test_epoch_end(self):
        ...
        if self.evaluation:
            ...
            if self.public_eval == True:
                self.hparams["data"]["test_tsv"] = self.hparams["data"]["public_eval_tsv"]
                self.hparams["data"]["test_dur"] = self.hparams["data"]["public_eval_dur"]
